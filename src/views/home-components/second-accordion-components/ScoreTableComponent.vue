...
export default {
  ...
  mounted() {
    if (sessionStorage.getItem("rows1")) {
      this.rows = JSON.parse(sessionStorage.getItem("rows1"));
      this.$store.state.rows = JSON.parse(sessionStorage.getItem("rows1"));
      // console.log(this.$store.state.rows);
    } else {
      sessionStorage.setItem("rows1", JSON.stringify(this.rows));
    }
  },
  ...
  watch: {
    rows: {
      deep: true,
      handler(newValue) {
        sessionStorage.setItem("rows1", JSON.stringify(newValue));
      },
    },
    totalObj: {
      deep: true,
      handler(newValue) {
        sessionStorage.setItem("totalObj1", JSON.stringify(newValue));
      },
    },
  },
  methods: {
