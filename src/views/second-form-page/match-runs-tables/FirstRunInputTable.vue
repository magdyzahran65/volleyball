...
<script>
export default {
  name: "FirstRunInputTable",
  mounted() {
    // T1T2
    if (sessionStorage.getItem("t1T2")) {
      this.t1T2 = JSON.parse(sessionStorage.getItem("t1T2"));
    } else {
      sessionStorage.setItem("t1T2", JSON.stringify(this.t1T2));
    }
    // ===t1T2Table2
    if (sessionStorage.getItem("t1T2Table2")) {
      this.t1T2Table2 = JSON.parse(sessionStorage.getItem("t1T2Table2"));
    } else {
      sessionStorage.setItem("t1T2Table2", JSON.stringify(this.t1T2Table2));
    }
    // =========================
    // runTables
    if (sessionStorage.getItem("runTableOne")) {
      this.table1 = JSON.parse(sessionStorage.getItem("runTableOne"));
    } else {
      sessionStorage.setItem("runTableOne", JSON.stringify(this.table1));
    }
    if (sessionStorage.getItem("runTableTwo")) {
      this.table2 = JSON.parse(sessionStorage.getItem("runTableTwo"));
    } else {
      sessionStorage.setItem("runTableTwo", JSON.stringify(this.table2));
    }
  ...
  watch: {
    ...
    table1: {
      ...
      handler(newValue) {
        ...
        sessionStorage.setItem("runTableOne", JSON.stringify(newValue));
      },
    },
    table2: {
      deep: true,
      handler(newValue) {
        // Store the updated DateTimeForm object in the session storage
        sessionStorage.setItem("runTableTwo", JSON.stringify(newValue));
      },
    },
    // t1t2
    t1T2: {
      ...
      handler(newValue) {
        // Store the updated DateTimeForm object in the session storage
        sessionStorage.setItem("t1T2", JSON.stringify(newValue));
      },
    },
    t1T2Table2: {
      deep: true,
      handler(newValue) {
        // Store the updated DateTimeForm object in the session storage
        sessionStorage.setItem("t1T2Table2", JSON.stringify(newValue));
      },
    },
  },
  methods: {
