<template>
  <div class="first-run-table">
    <h6
      class="text-decoration-underline text-white p-2 rounded-2 text-bg-info mt-2"
    >
      الشوط الخامس
    </h6>
    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-6 m-0">
          <span class="float-start fs-5 w-100 mb-2 text-secondary">
        ...
        <!-- second table -->
        <div class="col-md-6">
          <span class="float-end fs-5 w-100 mb-2 text-secondary">
            {{ DateTimeTable.team2 }}
          </span>
          <table class="table table-bordered">
            <thead>
...
export default {
  ...
  mounted() {
    ...
    // =========================
    // run5Tables
    if (sessionStorage.getItem("run5TableOne")) {
      this.table1 = JSON.parse(sessionStorage.getItem("run5TableOne"));
    } else {
      sessionStorage.setItem("run4TableOne", JSON.stringify(this.table1));
    }
    if (sessionStorage.getItem("run5TableTwo")) {
      this.table2 = JSON.parse(sessionStorage.getItem("run5TableTwo"));
    } else {
      sessionStorage.setItem("run5TableTwo", JSON.stringify(this.table2));
    }
  ...
  watch: {
    ...
    table1: {
      ...
      handler(newValue) {
        ...
        sessionStorage.setItem("run5TableOne", JSON.stringify(newValue));
      },
    },
    table2: {
      deep: true,
      handler(newValue) {
        // Store the updated DateTimeForm object in the session storage
        sessionStorage.setItem("run5TableTwo", JSON.stringify(newValue));
      },
    },
    // t1t2
    t1T2: {
      ...
      handler(newValue) {
        // Store the updated DateTimeForm object in the session storage
        sessionStorage.setItem("t1T2Run5", JSON.stringify(newValue));
      },
    },
    t1T2Table2: {
      deep: true,
      handler(newValue) {
        // Store the updated DateTimeForm object in the session storage
        sessionStorage.setItem("t1T2Run5Table2", JSON.stringify(newValue));
      },
    },
  },
  methods: {
