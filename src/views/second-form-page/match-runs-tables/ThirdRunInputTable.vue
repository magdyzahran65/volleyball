...
<script>
export default {
  name: "ThirdRunInputTable",
  mounted() {
    // T1T2Run3
    if (sessionStorage.getItem("t1T2Run3")) {
      this.t1T2 = JSON.parse(sessionStorage.getItem("t1T2Run3"));
    } else {
      sessionStorage.setItem("t1T2Run3", JSON.stringify(this.t1T2));
    }
    // ===t1T2Table2
    if (sessionStorage.getItem("t1T2Run3Table2")) {
      this.t1T2Table2 = JSON.parse(sessionStorage.getItem("t1T2Run3Table2"));
    } else {
      sessionStorage.setItem("t1T2Run3Table2", JSON.stringify(this.t1T2Table2));
    }
    // =========================
    // run3Tables
    if (sessionStorage.getItem("run3TableOne")) {
      this.table1 = JSON.parse(sessionStorage.getItem("run3TableOne"));
    } else {
      sessionStorage.setItem("run3TableOne", JSON.stringify(this.table1));
    }
    if (sessionStorage.getItem("run3TableTwo")) {
      this.table2 = JSON.parse(sessionStorage.getItem("run3TableTwo"));
    } else {
      sessionStorage.setItem("run3TableTwo", JSON.stringify(this.table2));
    }
  ...
  watch: {
    ...
    table1: {
      ...
      handler(newValue) {
        ...
        sessionStorage.setItem("run3TableOne", JSON.stringify(newValue));
      },
    },
    table2: {
      deep: true,
      handler(newValue) {
        // Store the updated DateTimeForm object in the session storage
        sessionStorage.setItem("run3TableTwo", JSON.stringify(newValue));
      },
    },
    // t1t2
    t1T2: {
      ...
      handler(newValue) {
        // Store the updated DateTimeForm object in the session storage
        sessionStorage.setItem("t1T2Run3", JSON.stringify(newValue));
      },
    },
    t1T2Table2: {
      deep: true,
      handler(newValue) {
        // Store the updated DateTimeForm object in the session storage
        sessionStorage.setItem("t1T2Run3Table2", JSON.stringify(newValue));
      },
    },
  },
  methods: {
